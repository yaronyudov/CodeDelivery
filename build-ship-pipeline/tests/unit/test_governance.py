...
def test_dynamic_caps_tighten_as_budget_depletes():
    ...
    s_full = _state(
        tokens_used=0, cost_used=0.0, steps_taken=0, steps_limit=10,
        tokens_limit=100_000, cost_limit=0.20,
    )
    # burned: 8 steps used, but burned 95K tokens/$0.19 (fast burn)
    # only 5K tokens and $0.01 left for the last 2 steps
    s_burned = _state(
        tokens_used=95_000, cost_used=0.19, steps_taken=8, steps_limit=10,
        tokens_limit=100_000, cost_limit=0.20,
    )
...
def test_all_roles_have_weights():
    for role in ["planner", "coder", "docker", "observability", "tester",
                 "debugger", "reviewer", "review_sup", "security", "perf",
                 "style", "coverage"]:
