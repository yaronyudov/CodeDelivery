...
def test_bm25_returns_most_relevant_doc():
    ...
    retriever.add_documents(
        [
            Document(
                id="auth", content="JWT authentication token bearer cookie security", metadata={}
            ),
            Document(
                id="db", content="PostgreSQL database connection pool psycopg query", metadata={}
            ),
            Document(
                id="docker", content="Docker container compose service nginx proxy", metadata={}
            ),
        ]
    )
...
def test_bm25_respects_k():
    ...
    for i in range(10):
        retriever.add_documents(
            [Document(id=f"d{i}", content=f"document about topic {i} keyword", metadata={})]
        )
